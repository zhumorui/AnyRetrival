...
def download_datasets(data_dir):
    ...
    for di in range(len(datasets)):
        ...
        if not os.path.isdir(dst_dir):
            print(
                ">> Dataset {} directory does not exist. Creating: {}".format(
                    dataset, dst_dir
                )
            )
            os.makedirs(dst_dir)
            for dli in range(len(dl_files)):
                ...
                print(
                    ">> Downloading dataset {} archive {}...".format(dataset, dl_file)
                )
                ...
                print(
                    ">> Extracted, deleting dataset {} archive {}...".format(
                        dataset, dl_file
                    )
                )
        ...
        gnd_src_dir = os.path.join(
            "http://cmp.felk.cvut.cz/revisitop/data", "datasets", dataset
        )
    ...
    if not os.path.isdir(dst_dir):
        ...
        for dfi in range(nfiles):
            ...
            if os.path.exists(dst_file):
                print(
                    ">> [{}/{}] Skipping dataset {} archive {}, already exists...".format(
                        dfi + 1, nfiles, dataset, dl_file
                    )
                )
            else:
                while 1:
                    try:
                        print(
                            ">> [{}/{}] Downloading dataset {} archive {}...".format(
                                dfi + 1, nfiles, dataset, dl_file
                            )
                        )
        ...
        for dfi in range(nfiles):
            ...
            dst_file = os.path.join(dst_dir_tmp, dl_file)
            print(
                ">> [{}/{}] Extracting dataset {} archive {}...".format(
                    dfi + 1, nfiles, dataset, dl_file
                )
            )
...
def download_features(data_dir):
    ...
    for di in range(len(datasets)):
        ...
        if not os.path.exists(feat_dst_file):
            print(
                ">> Downloading dataset {} features file {}...".format(
